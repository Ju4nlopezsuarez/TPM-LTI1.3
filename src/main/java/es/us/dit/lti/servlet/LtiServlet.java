...
public class LtiServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idToken != null) {
            ...
            response.sendError(HttpServletResponse.SC_BAD_REQUEST,
                    "Invalid LTI Request: Missing id_token or oauth_consumer_key.");
        }
    }
     ...
     */
    private void handleLti13(HttpServletRequest request, HttpServletResponse response, String idToken)
            throws IOException {
        try {

            // Debemos comprobar que el state recibido coincide con el que generamos en
            // OIDCLoginServlet
            ...
            if (storedState == null || !storedState.equals(receivedState)) {
                ...
                response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                        "Security Error: Invalid State (CSRF). Try refreshing the LMS page.");
                ...
                if (audiences == null || audiences.isEmpty()) {
                    ...
                    if (config == null || !config.getIssuer().equals(issuer)) {
                        logger.error("LTI 1.3 Error: Unknown Configuration for client_id {} and issuer {}", clientId,
                                issuer);
                        response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                                "Tool not configured for Issuer: " + issuer);

                    } else {

                        // Usamos SecurityUtil para descargar las claves del LMS y verificar
                        JWTClaimsSet claims = SecurityUtil.validateLti13Token(idToken, config.getJwksUrl(), issuer,
                                config.getClientId());
                        ...
                        if (storedNonce == null || !storedNonce.equals(tokenNonce)) {
                            ...
                            response.sendError(HttpServletResponse.SC_UNAUTHORIZED,
                                    "Invalid Nonce (CSRF). Try refreshing the LMS page.");

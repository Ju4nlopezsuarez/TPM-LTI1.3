...
public class EditPlatformServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (idStr != null && issuer != null && !issuer.trim().isEmpty()) {
            try {
                ...
                boolean updated = dao.update(platform);
                
                if (updated) {
                    request.getSession().setAttribute(Settings.PENDING_MSG_ATTRIB, "Plataforma actualizada con éxito");
                } else {
                    request.getSession().setAttribute(Settings.PENDING_MSG_ATTRIB, "Error al actualizar la plataforma");
                }
